...
describe("wrapWaypoints (issues #206, #307)", () => {
  ...
  test("re-offsets snapped route.waypoints back to original coordinate space", () => {
    ...
    const router = {
      route: function (wps, cb) {
        ...
        const snapped = [{ latLng: { lat: 53.27, lng: -2.8 }, name: "" }];
    ...
    expect(receivedRoutes[0].waypoints[0].latLng.lng).toBeCloseTo(-362.8);
  });
  ...
  test("handles null waypoints gracefully", () => {
    ...
    expect(() =>
      router.route(
        [null, { latLng: makeLatLng(0, 0), name: "" }],
        function () {},
      ),
    ).not.toThrow();
  ...
  test("re-offsets route.coordinates so the polyline draws in the correct world copy", () => {
    ...
    const router = {
      route: function (wps, cb) {
        cb(null, [
          {
            waypoints: [{ latLng: { lat: 39.9, lng: 116.39 }, name: "" }],
            coordinates: coords,
          },
        ]);
      },
  ...
  test("does not re-offset route.coordinates when coords are already in range", () => {
    ...
    const router = {
      route: function (wps, cb) {
        cb(null, [
          {
            waypoints: [{ latLng: { lat: 48.8, lng: 2.3 }, name: "" }],
            coordinates: coords,
          },
        ]);
      },
    ...
    expect(receivedRoutes[0].coordinates[0].lng).toBeCloseTo(2.3);
  });
});
