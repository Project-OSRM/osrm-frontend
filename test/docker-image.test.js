...
test("docker image serves a page (index.html)", async () => {
  ...
  await exec(`docker build -f docker/Dockerfile -t ${IMAGE_TAG} .`, {
    timeout: 3 * 60 * 1000,
    maxBuffer: 10 * 1024 * 1024,
  });

  // Run detached with random host port published for exposed ports
  const { stdout: runOut } = await exec(
  ...
  try {
    ...
    const { stdout: portOut } = await exec(
      `docker port ${containerId} 9966/tcp`,
    );
    ...
    try {
      await exec(`docker rmi -f ${IMAGE_TAG}`);
    } catch (e) {}
  }
});
