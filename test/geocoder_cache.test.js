...
describe("geocoder cache", function () {
  ...
  function makeLeafletMock() {
    return {
      Control: {
        Geocoder: {
          nominatim: jest.fn(function () {
            return {
              geocode: jest.fn(function () {
                return Promise.resolve([]);
              }),
              reverse: jest.fn(function () {
                return Promise.resolve([]);
              }),
            };
          }),
  ...
  test("caches successful search responses and reuses cache", async function () {
    ...
    jest.doMock("leaflet", function () {
      ...
      m.Control.Geocoder.nominatim = jest.fn(function () {
        return {};
      });
      return m;
    });
    ...
    var expectedUrl =
      "https://nominatim.example/search?format=json&addressdetails=1&limit=5&q=" +
      encodeURIComponent("Somewhere");
    var found = entries.find(function (e) {
      return e[0] === expectedUrl;
    });
  ...
  test("expires old entries on load (TTL)", async function () {
    ...
    var stored = [
      [
        url,
        { value: [{ name: "Old", center: { lat: 11, lng: 22 } }], ts: oldTs },
      ],
    ];
  ...
  test("sliding TTL: accessing an entry refreshes its expiry", async function () {
    ...
    var stored = [
      [
        url,
        {
          value: [{ name: "Sliding", center: { lat: 1, lng: 2 } }],
          ts: almostExpiredTs,
        },
      ],
    ];
  ...
  test("evicts oldest entry when capacity exceeded", async function () {
    ...
    for (var i = 0; i < 128; i++) {
      var u =
        service +
        "search?format=json&addressdetails=1&limit=5&q=" +
        encodeURIComponent("q" + i);
      entries.push([
        u,
        { value: [{ name: "P" + i, center: { lat: i, lng: i } }], ts: now },
      ]);
    }
    ...
    var firstExpected =
      service +
      "search?format=json&addressdetails=1&limit=5&q=" +
      encodeURIComponent("q0");
    expect(
      arr.find(function (e) {
        return e[0] === firstExpected;
      }),
    ).toBeUndefined();
    var newUrl =
      service +
      "search?format=json&addressdetails=1&limit=5&q=" +
      encodeURIComponent("NewPlace");
    expect(
      arr.find(function (e) {
        return e[0] === newUrl;
      }),
  ...
  test("uses nominatim geocode path and caches results", async function () {
    jest.resetModules();
    var geocodeMock = jest.fn().mockResolvedValue([
      {
        name: "NPlace",
  ...
  test("caches nominatim.reverse results and reuses them", async function () {
    jest.resetModules();
    var reverseMock = jest.fn().mockResolvedValue([
      {
        name: "RPlace",
    ...
    var nominatimFactory = jest.fn(function () {
      return { reverse: reverseMock };
    });
    jest.doMock("leaflet", function () {
      return {
