...
describe("State language updates", function () {
  it("updates geocoder placeholders when language changes", function () {
    ...
    plan._geocoderElems = [
      {
        options: { language: "en" },
        _element: { input: document.createElement("input") },
      },
      {
        options: { language: "en" },
        _element: { input: document.createElement("input") },
    ...
    expect(
      plan._geocoderElems[0]._element.input.getAttribute("placeholder"),
    ).toBe("de-start");
    expect(
      plan._geocoderElems[1]._element.input.getAttribute("placeholder"),
    ).toBe("de-via");
    expect(
      plan._geocoderElems[2]._element.input.getAttribute("placeholder"),
    ).toBe("de-end");
  });
});
