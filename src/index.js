...
for (var i = 0, len = services.length; i < len; i++) {
  ...
  services[i].label =
    localization.t(language, profileLabelKey) || profileLabelKey;
}
var modeSelector = modeSelectorModule.createModeSelector(
...
var baselayer = ls.get("layer")
  ...
  : leafletOptions.defaultState.layer;
...
var _initResult = initialLayers.determineInitialLayers(
  baselayer,
  overlay,
  services,
  _initProfileIndex,
...
var scaleControl = L.control
  .scale({
    ...
    metric:
      mergedOptions.units === "metric" || mergedOptions.units === undefined,
...
var plan = new ReversablePlan([], {
  geocoder: createGeocoder.coordPreserving(
    leafletOptions.nominatim && leafletOptions.nominatim.path,
  ),
  ...
  geocoderPlaceholder: function (i, n, geocoderElement) {
    var activeLanguage =
      geocoderElement && geocoderElement.options
        ? geocoderElement.options.language
        : mergedOptions.language;
    var startend = [
      localization.t(activeLanguage, "Start - press enter to drop marker"),
      localization.t(activeLanguage, "End - press enter to drop marker"),
    ];
    var via = [
      localization.t(activeLanguage, "Via point - press enter to drop marker"),
    ];
    if (i === 0) {
      return startend[0];
    }
...
if (
  L &&
  L.DomEvent &&
  typeof L.DomEvent.disableScrollPropagation === "function"
) {
  var routingContainers = document.querySelectorAll(
...
var toolsControl = tools
  .control(
    ...
    Object.assign({}, options.tools, { initialUnits: mergedOptions.units }),
  )
  .addTo(map);
  ...
  toolsControl.on("unitschanged", function (e) {
    try {
      if (lrmControl) {
        ...
        if (
          lrmControl._formatter &&
          typeof lrmControl._formatter === "object"
        ) {
...
if (modeSelector && modeSelector.select) {
  L.DomEvent.on(modeSelector.select, "change", function (event) {
    var profileIndex = parseInt(event.target.value, 10);
    var selectedProfile =
      services[profileIndex] && services[profileIndex].profile;
...
(function applyAddressParams() {
  var hasLocWaypoints =
    mergedOptions.waypoints &&
    mergedOptions.waypoints.some(function (wp) {
      return wp && wp.latLng;
    });
...
lrmControl.on("routeselected", function (e) {
  ...
  if (
    container &&
    !container.classList.contains("leaflet-routing-container-hide")
  ) {
  ...
  if (currentZoom >= 13) {
    ...
    if (
      routePixelWidth <= availableWidth &&
      routePixelHeight <= availableHeight
    ) {
