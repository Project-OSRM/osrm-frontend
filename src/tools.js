...
var Control = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    this._shareButton = L.DomUtil.create(
      "span",
      this.options.shareButtonClass,
      shareContainer,
    );
  ...
  _showSharePopup: function () {
    ...
    this._sharePopup = L.DomUtil.create(
      "div",
      "share-popup fill-osrm dark",
      this._shareContainer,
    );
    ...
    typeButtonContainer = L.DomUtil.create(
      "div",
      "share-type-button-container",
      container,
    );
    linkButton = L.DomUtil.create(
      "button",
      "share-type selected",
      typeButtonContainer,
    );
    ...
    shortLinkButton = L.DomUtil.create(
      "button",
      "share-type",
      typeButtonContainer,
    );
  ...
  _createUnitsChooser: function (container) {
    var _this = this;
    var unitsSelect = L.DomUtil.create(
      "select",
      _this.options.localizationChooserClass,
      container,
    );
    unitsSelect.setAttribute(
      "title",
      _this._local["Select units"] || "Select units",
    );
    L.DomEvent.on(
      unitsSelect,
      "change",
    ...
    metricOption.appendChild(
      document.createTextNode(_this._local["Metric"] || "Metric"),
    );

    var imperialOption = L.DomUtil.create("option", "fill-osrm", unitsSelect);
    imperialOption.setAttribute("value", "imperial");
    imperialOption.appendChild(
      document.createTextNode(_this._local["Imperial"] || "Imperial"),
    );
  ...
  _createVersionInfo: function (container) {
    ...
    var versionContainer = L.DomUtil.create(
      "div",
    ...
    infoIcon.setAttribute(
      "data-tooltip",
      this._local["Build"] + ": " + versionInfo.timestamp,
    );
